...
# def bbox_iou(box1, box2, xywh=True, GIoU=False, DIoU=False, CIoU=False, SIoU=False, EIoU=False, MPDIoU=False, Focal=False, alpha=1, gamma=0.5, eps=1e-7):
...
#     if CIoU or DIoU or GIoU or EIoU or SIoU or MPDIoU:
#         cw = b1_x2.maximum(b2_x2) - b1_x1.minimum(b2_x1)  # convex (smallest enclosing box) width
#         ch = b1_y2.maximum(b2_y2) - b1_y1.minimum(b2_y1)  # convex height
#         if CIoU or DIoU or EIoU or SIoU or MPDIoU:  # Distance or Complete IoU https://arxiv.org/abs/1911.08287v1
...
#                 rho_y = (s_ch / ch) ** 2
#                 gamma1 = angle_cost - 2
#                 distance_cost = 2 - torch.exp(gamma1 * rho_x) - torch.exp(gamma1 * rho_y)
...
#             elif MPDIoU:
#                 d1 = ((b2_x1 - b1_x1) ** 2 + (b2_y1 - b2_y1) ** 2) / (w ** 2 + h ** 2)
#                 d2 = ((b2_x2 - b1_x2) ** 2 + (b2_y2 - b2_y2) ** 2) / (w ** 2 + h ** 2)
#                 if Focal:
#                     return iou - d1 - d2, torch.pow(inter/(union + eps), gamma) # Focal_MPDIou
#                 else:
#                     return iou - d1 - d2 # MPDIou
#             if Focal:
...
#             keep.append(i)
#
#         iou = bbox_iou_for_nms(bboxes[i], bboxes[order[1:]], SIoU=True).squeeze()
#
