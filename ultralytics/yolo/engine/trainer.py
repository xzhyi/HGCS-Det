...
from ultralytics.nn.modules import GlobalLocalFilter, SpectralGatingNetwork, BiFPN_Add2, BiFPN_Add3, BiFPN_Concat2, BiFPN_Concat3
    ...
    def build_optimizer(model, name='Adam', lr=0.001, momentum=0.9, decay=1e-5):
        ...
        for v in model.modules():
            ...
            if isinstance(v, bn):  # weight (no decay)
                ...
                g[0].append(v.weight)
            elif hasattr(v, 'theta') and isinstance(v.theta, nn.Parameter):  # weight (with decay)
                g[0].append(v.theta)
            # elif isinstance(v, GlobalLocalFilter) and hasattr(v, 'complex_weight') and isinstance(v.complex_weight, nn.Parameter):
            #     g[0].append(v.complex_weight)
            # elif isinstance(v, SpectralGatingNetwork) and hasattr(v, 'complex_weight') and isinstance(v.complex_weight, nn.Parameter):
            #     g[0].append(v.complex_weight)
            # # BiFPN_Concat
